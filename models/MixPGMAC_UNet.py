...
class MixPGMAC_UNet(nn.Module):
    def __init__(self, band_num, class_num):
        super(MixPGMAC_UNet, self).__init__()
        ...
        self.conv1 = nn.Sequential(
            GACBlock(self.band_num, channels[0]),
            GACBlock(channels[0], channels[0])
        )
        self.conv12 = nn.Sequential(
            MixPool(kernel_size=(2, 2), stride=(2, 2), padding=0, aplha=1),
        ...
        self.conv2 = nn.Sequential(
            MixPool(kernel_size=(2, 2), stride=(2, 2), padding=0, aplha=0.6),
            ACBlock(channels[0], channels[1]),
        ...
        self.conv3 = nn.Sequential(
            MixPool(kernel_size=(2, 2), stride=(2, 2), padding=0, aplha=0.4),
            ACBlock(channels[1], channels[2]),
        ...
        self.conv4 = nn.Sequential(
            MixPool(kernel_size=(2, 2), stride=(2, 2), padding=0, aplha=0.2),
            ACBlock(channels[2], channels[3]),
