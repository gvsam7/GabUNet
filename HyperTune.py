...
def quick_test():
    args = arguments()
    args.epochs = 2  # Run for only 2 epochs
    args.batch_size = 4  # Use smaller batch size for quick testing

    # Define hyperparameter search space (only a few combinations for quick test)
    learning_rates = [1e-4]
    batch_sizes = [4]
    num_layers = [8]
    hidden_dims = [512]
    ...
    for lr, bs, nl, hd, nh, dr, ps in itertools.product(learning_rates, batch_sizes, num_layers, hidden_dims, num_heads,
        ...
        # Data split (smaller subsets for quick test)
